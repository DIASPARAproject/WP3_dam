-- ***************************************************
-- Creating schema France structure
-- tables, trigger constraints, primary and foreign keys
-- *************************************************

-- ***************************************************
-- 1 - France.establishment
-- ***************************************************
-- ***************************************************
-- 2 - France.data_provider
-- ***************************************************
-- ***************************************************
-- 3 - France.France.observation_places
-- ***************************************************
-- ***************************************************
-- 4 - France.aspe_station_op
-- ***************************************************
-- ***************************************************
-- 5 - France.observations
-- ***************************************************
-- ***************************************************
-- 6 - France.electrofishing;
-- ***************************************************
-- ***************************************************
-- 7 - France.aspe_operation_ef
-- ***************************************************
...
ALTER TABLE France.establishment OWNER TO dbmig_FranceManager;
GRANT SELECT ON TABLE France.establishment TO diaspara_read;

CREATE TRIGGER tr_pk_id_establishment_insert BEFORE INSERT OR UPDATE ON France.establishment FOR EACH ROW EXECUTE FUNCTION dbmig.pk_id_establishment_insert();

CREATE TRIGGER tr_establishment_delete
BEFORE DELETE ON France.establishment
FOR EACH ROW EXECUTE FUNCTION dbmig.integrity_establishment_verify();
...
) INHERITS (dbmig.data_provider);
ALTER TABLE France.data_provider
    ADD CONSTRAINT FRA_data_provider_pkey PRIMARY KEY (dp_id);
ALTER TABLE France.data_provider OWNER TO dbmig_FranceManager;
GRANT SELECT ON TABLE France.data_provider TO diaspara_read;

CREATE TRIGGER tr_pk_id_data_provider_insert BEFORE INSERT OR UPDATE ON France.data_provider FOR EACH ROW EXECUTE FUNCTION dbmig.pk_id_data_provider_insert();

CREATE TRIGGER tr_fk_id_establishment_insert
BEFORE INSERT OR UPDATE ON France.data_provider
FOR EACH ROW EXECUTE FUNCTION dbmig.fk_id_establishment_insert();

-- ***************************************************
-- 3 - France.observation_places
-- ***************************************************
...
) INHERITS (dbmig.observation_places);
...
ALTER TABLE France.observation_places OWNER TO dbmig_FranceManager;
GRANT SELECT ON TABLE France.observation_places TO diaspara_read;

--- @label primary key workaround
CREATE TRIGGER tr_observation_places_insert BEFORE INSERT ON France.observation_places FOR EACH ROW EXECUTE FUNCTION dbmig.observation_id_place_insert();

CREATE TRIGGER tr_observation_places_update BEFORE UPDATE ON France.observation_places FOR EACH ROW EXECUTE FUNCTION dbmig.observation_id_place_update();

CREATE TRIGGER tr_observation_places_delete
BEFORE DELETE ON France.observation_places
FOR EACH ROW EXECUTE FUNCTION dbmig.integrity_observation_places_verify();

--- @label foreign keys
ALTER TABLE France.observation_places
    ADD CONSTRAINT FRA_fk_op_no_observationplacetype FOREIGN KEY (op_no_observationplacetype) REFERENCES dbmig_nomenclature.observation_place_type(no_id) ON UPDATE CASCADE ON DELETE RESTRICT;
ALTER TABLE France.observation_places
...
CREATE TRIGGER tr_fk_id_data_provider_insert BEFORE INSERT OR UPDATE ON France.observation_places FOR EACH ROW EXECUTE FUNCTION dbmig.fk_id_data_provider_insert();
...
--- @label owner and privileges
ALTER TABLE France.aspe_station_op OWNER TO dbmig_FranceManager;
GRANT SELECT ON TABLE France.aspe_station_op TO diaspara_read;

--- @label primary key workaround
CREATE TRIGGER tr_aspe_station_op BEFORE INSERT ON France.aspe_station_op FOR EACH ROW EXECUTE FUNCTION dbmig.observation_id_place_insert();

CREATE TRIGGER tr_aspe_station_op_update BEFORE UPDATE ON France.aspe_station_op FOR EACH ROW EXECUTE FUNCTION dbmig.observation_id_place_update();
...
--- @label foreign keys workaround
CREATE TRIGGER tr_fk_id_data_provider_insert BEFORE INSERT OR UPDATE ON France.aspe_station_op FOR EACH ROW EXECUTE FUNCTION dbmig.fk_id_data_provider_insert();


-- ***************************************************
-- 4 - France.rsa_station_op
-- ***************************************************
--- @label table creation
...
--- @label owner and privileges
ALTER TABLE France.rsa_station_op OWNER TO dbmig_FranceManager;
GRANT SELECT ON TABLE France.rsa_station_op TO diaspara_read;

--- @label primary key workaround
CREATE TRIGGER tr_rsa_station_op BEFORE INSERT ON France.rsa_station_op FOR EACH ROW EXECUTE FUNCTION dbmig.observation_id_place_insert();

CREATE TRIGGER tr_rsa_station_op_update BEFORE UPDATE ON France.rsa_station_op FOR EACH ROW EXECUTE FUNCTION dbmig.observation_id_place_update();
...
--- @label foreign keys workaround
CREATE TRIGGER tr_fk_id_data_provider_insert BEFORE INSERT OR UPDATE ON France.rsa_station_op FOR EACH ROW EXECUTE FUNCTION dbmig.fk_id_data_provider_insert();
...
--- @label owner and privileges
ALTER TABLE France.observations OWNER TO dbmig_FranceManager;
...
CREATE TRIGGER tr_observations_delete
    ...
    FOR EACH ROW EXECUTE FUNCTION dbmig.integrity_observations_verify();

CREATE TRIGGER tr_observations_insert
    BEFORE INSERT ON France.observations
    FOR EACH ROW EXECUTE FUNCTION dbmig.observations_id_insert();

CREATE TRIGGER tr_observations_update
    BEFORE UPDATE ON France.observations
    FOR EACH ROW EXECUTE FUNCTION dbmig.observations_id_update();
...
ALTER TABLE France.observations
    ADD CONSTRAINT FRA_fk_ob_origin FOREIGN KEY (ob_no_origin) REFERENCES dbmig_nomenclature.observation_origin(no_id) ON UPDATE CASCADE ON DELETE RESTRICT;
ALTER TABLE France.observations
    ADD CONSTRAINT FRA_fk_ob_period FOREIGN KEY (ob_no_period) REFERENCES dbmig_nomenclature.period_type(no_id) ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE France.observations
    ADD CONSTRAINT FRA_fk_ob_type FOREIGN KEY (ob_no_type) REFERENCES dbmig_nomenclature.observation_type(no_id) ON DELETE RESTRICT ON UPDATE CASCADE;

--- @label foreign keys workaround
CREATE TRIGGER tr_observations_op_id_insert
    BEFORE INSERT OR UPDATE ON France.observations
    FOR EACH ROW EXECUTE FUNCTION dbmig.fk_id_observations_insert();
...
CREATE TABLE France.electrofishing(
    -- VOIR S'IL FAUT METTRE AUTRE CHOSE
) INHERITS (dbmig.electrofishing, France.observations); -- à noter le double héritage
...
--- @label owner and privileges
ALTER TABLE France.electrofishing OWNER TO dbmig_FranceManager;
...
ALTER TABLE France.electrofishing
    ADD CONSTRAINT FRA_fk_ob_type FOREIGN KEY (ob_no_type) REFERENCES dbmig_nomenclature.observation_type(no_id) ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE France.electrofishing
    ADD CONSTRAINT FRA_fk_ef_electrofishing_mean FOREIGN KEY (ef_no_electrofishing_mean) REFERENCES dbmig_nomenclature.electrofishing_mean(no_id) ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE France.electrofishing
    ADD CONSTRAINT FRA_fk_ef_electrofishing_method FOREIGN KEY (ef_no_fishingmethod) REFERENCES dbmig_nomenclature.scientific_observation_method(no_id) ON DELETE RESTRICT ON UPDATE CASCADE;
...
--- @label owner and privileges
ALTER TABLE France.aspe_operation_ef OWNER TO dbmig_FranceManager;
